...
If resHandler.isTenpaySign() = True Then
	...
	If "1" = trade_mode Then '即时到帐
		If "0" = trade_state Then
			...
			Response.Write("<br>订单"&out_trade_no&"即时到帐付款成功，我们会尽快处理订单业务，谢谢合作！")'即时到帐支付成功Success
	    '处理成功
		Else  
	...
	Else
		If "2"= trade_mode  Then    ' 中介担保 
			If  "0"= trade_state  Then
				...
				Response.Write("<br>订单"&out_trade_no&"中介担保付款成功，我们会尽快处理订单业务，请及时确认付款，谢谢合作！")'支付成功Success"trade_state=" + trade_state
			Else 
