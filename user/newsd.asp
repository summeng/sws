...
<% 
'天天供求信息网店远程调用代码
'将此调用代码放在任何页面：<script language="javascript" src="网址及路径/user/newsd.asp?c1=0&c2=0&c3=0&t1=0&t2=0&t3=0&gdxx=0&btw=24&s=10&l=1&zt=13&hg=10"></script>

'调用参数说明。依次：c1地区一级分类（0不分、其它按该类ID号），c2地区二级分类（0不分、其它按该类ID号），c3地区三级分类（0不分、其它按该类ID号）,t1行业一级分类（0不分、其它按该类ID号）,t2行业二级分类（0不分、其它按该类ID号）,t3行业三级分类（0不分、其它按该类ID号）,gdxx更多（0不显1显），btw名称长度（以字节计），s显示条数，l列数，zt字号，hg行高
...
sql="select * from [DNJY_user] where sdname<>'' and sdkg=1"
sql=sql&""&ttcc
sql=sql&""&tttt
...
TempText=TempText+"<a target='_blank' href='http://<%=web%>/user/com.asp?com=<%=rs("username")%>'>"
TempText=TempText+"<SPAN style='font-size=<%=zt%>px'><%=CutStr(rs("sdname"),btw,"...")%></SPAN></a>"
TempText=TempText+"</TD>"
<%iii=iii+1
IF iii Mod l=0 then%>
...
if i>=s then exit Do
	...
	<%End IF%>	
	<%If gdxx=1 Then%>
	TempText=TempText+"<tr height='<%=hg%>'><td align='right'><A href='http://<%=web%>/sdlist.asp?c=<%=c1%>,<%=c2%>,<%=c3%>&t=<%=t1%>,<%=t2%>,<%=t3%>' target=_blank><SPAN style='font-size=<%=zt%>px'>更多>>></SPAN></a></td></tr>"
<%End If%>
