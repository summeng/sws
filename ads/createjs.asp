...
<%call checkmanage("13")%>
<%dim objStream
	Dim adkg,ad,ADID,HostURL,ScrCode,JsCode,AD_ADHits,ADjs
...
Call OpenConn
	Set rs=server.createobject("adodb.recordset")
		sql = "Select adkg,ADType,ADSrc,ADLink,ADWidth,ADHeight,ADAlt,ADViews,ADHits,ADStopViews,ADStopHits,ADjs,ADStopDate,city_oneid,city_twoid,city_threeid from [DNJY_ad] where ADID='" & ADID & "' and city_oneid="&city_oneid&" and city_twoid="&city_twoid&" and city_threeid="&city_threeid&""
	...
	If Not (rs.bof or rs.eof) Then
		...
		ADjs=rs("ADjs")		
		If ad="zt" Then rs("adkg")=0
		If ad="hd" Then rs("adkg")=1
		adkg=rs("adkg")
...
If ad="zt" Or adkg=0 Then'================m=================
JsCode="document.write(' <!--广告暂停 ID:"&ADID&"_"&city_oneid&"_"&city_twoid&"_"&city_threeid&" -->')"
else
...
If ad="zt" Or ad="hd" Then
response.Write "<script language=javascript>location.replace('"&request("url")&"?page="&request("page")&"');</script>"
response.End
End if
	If Err <> 0 Then
		Response.Write ("<script>alert(' 刷新JS代码失败! \n\n "& Err.Description &"');history.back();</script>")
	Else
		Response.Write ("<script>alert(' 刷新JS代码成功 !');history.back();</script>")
	End If
	Else
