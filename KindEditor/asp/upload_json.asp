...
Dim aspUrl, savePath, saveUrl, maxSize, fileName, fileExt, newFileName, filePath, fileUrl, dirName,action
...
saveUrl = "/"&strInstallDir&"UploadFiles/"
'定义允许上传的文件扩展名
imageExtStr = "gif|jpg|jpeg|png|bmp|swf|flv|mp3|wav|wma|wmv|mid|avi|mpg|asf|rm|rmvb|doc|docx|xls|xlsx|ppt|htm|html|txt|zip|rar|gz|bz2"
'flashExtStr = "swf|flv"
...
If action="infopic" Then
dirName = "infopic"'信息内容文件存放目录
ElseIf action="upnews" Then
dirName = "upnews"'新闻资讯文件存放目录
ElseIf action="photo" Then
dirName = "photo"'图文文件存放目录
ElseIf action="usernews" Then
dirName = "usernews"'会员资讯文件存放目录
ElseIf action="help" Then
dirName = "help"'帮助文件存放目录
else
End if
If instr(lcase("infopic,upnews,photo,usernews,help"), dirName) < 1 Then
	showError("目录名不正确。")
End If
