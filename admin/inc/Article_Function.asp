...
Sub Admin_ShowItem_Name(ItemID)   
   ...
   ItemID=Clng(ItemID)
   Sqlc ="select top 1 ItemName from DNJY_Article_Item Where ItemID=" & ItemID   
...
Sub Admin_ShowItem_Option(ItemID)   
   ...
   ItemID=Clng(ItemID)
   SqlI ="select ItemID,ItemName from DNJY_Article_Item order by ItemID desc"   
...
dim http
set http=server.createobject("MSXML2.ServerXMLHttp")'使用xmlhttp的方法来获得图片的内容
...
'参  数: TistUrl------ 当前网页地址
'==================================================
Function ReplaceSaveRemoteFile(ConStr,cj_strInstallDir,strChannelDir,SaveTf,TistUrl)'内容，当前工作目录，保存目录名，确认保存，图片来源地址
   If ConStr="$False$" or ConStr="" or cj_strInstallDir="" or strChannelDir="" Then
   ...
   If SaveTf=True then
 '***********************************
      SavePath= "/"&strInstallDir&"UploadFiles/"&strChannelDir & "/"'图片保存路径 
...
'********************************
            ...
            Re.Pattern="/"&cj_strInstallDir & strChannelDir & "/"
            UploadFiles=UploadFiles & "|" & Re.Replace(SavePath &strFileName,"")
'=========================给图片加水印开始=========================
if SY_Type>0 and IsObjInstalled("Persits.Jpeg") And cj_watermark=True then 
Call PicAspJpeg(PathTemp)
End if
'=========================给图片加水印END=========================
