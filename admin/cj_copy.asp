...
If FoundErr<>True Then
   Set Rs=conn.execute("Select * from DNJY_Article_Item Where ItemID=" & ItemID)
   ...
   If IsArray(Arr_Item)=True Then
      ...
      sql="select top 1 * from DNJY_Article_Item" 
      ...
      If IsArray(Arr_Filter)=True Then
         set rs=server.createobject("adodb.recordset")
         sql="select top 1 * from DNJY_Article_Filters" 
         rs.open sql,conn,1,3
         rs.AddNew
            rs(1)=ItemID
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="2" class="admintable">
  <tr>
    <td height="30" class="b1_1" style="text-align:center;padding:50px;"><%=ItemName%> 项目复制完成，新的项目保存为：<%=ItemName%>复件<br><br><a href="cj_start.asp">返回</a></td>
  </tr>
</table>
<%End Sub%>
</body>         
