...
set rs=server.CreateObject("adodb.recordset")
 rs.open "select * from DNJY_hy_type where name='"&dname&"' and threeid=0 and twoid=0",conn,1,1
    if rs.EOF and rs.BOF then
    rs.close
    rs.open "select * from DNJY_hy_type where threeid=0 and twoid=0 order by id "&DN_OrderType&"",conn,1,1
	if rs.eof and rs.bof then
	id=1
	else
    id=rs("id")+1
...
case "edit"
   set rs=server.CreateObject("adodb.recordset")
	rs.open "select name from DNJY_hy_type where twoid=0 and threeid=0  and name='"&dname&"' and i<>"&idd&"",conn,1,1
	if not rs.eof Then
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from DNJY_hy_type where twoid=0 and id="&id,conn,1,3
yname=rs("name")
rs("name")=dname
rs("indexid")=strint(request("indexid"))
rs("indexshow")=request("indexshow")
...
case "del"
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_hy_type where twoid>0 and id ="&id
...
response.End
End If
conn.execute ("delete from DNJY_hy_type where id="&id&"")
'删除关联分类的信息及相关文件开始
...
Call CloseDB(conn)
'删除关联分类的信息及相关文件END
response.Redirect "store_Level1.asp" 
end select
%>
<HTML>
<HEAD>
...
<table width="98%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#335078" bordercolordark="#FFFFFF" bgcolor="#C5D5E4"  style="font-size:12px; letter-spacing:1px;">
  <tr> 
    <td height="20" bgcolor="#C5D5E4" align="center"><span style="color: #FF0000"><strong><font style="font-size:14px">一 级 分 类 栏 目 管 理</font></strong></span></td>
  </tr>
  <TR> 
    <TD bgcolor="#EAEEF4">&nbsp;[<A href="store_Level1.asp">首页</A>]
	<table width="98%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#335078" bordercolordark="#FFFFFF" bgcolor="#C5D5E4"  style="font-size:12px; letter-spacing:1px;">
        <TR align="center" bgcolor="#FFFFFF" height="20">          
		  <TD width="40" bgcolor="#D9E4EE">调用ID</TD>
        ...
        <%set rs=server.CreateObject("adodb.recordset")
		  rs.Open "select * from DNJY_hy_type where twoid=0 and id>0 order by id",conn,1,1		  
		  if rs.EOF and rs.BOF then
          response.write"<tr bgcolor=#FFFFFF><td colspan='6'><p align='center'><font color='red'>暂无分类！请先添加分类。如果觉得官方的默认分类合用，可</font><a href=""Classify_in_out.asp?Action=dqfl"" TARGET=right><FONT color=""#0000FF"" >导入官方默认店企分类。</font></a>如果不合用请勿导入，否则一个个删除非常麻烦！<a href=""http://test.ip126.com/sdindex.asp"" target=_blank><FONT color=""#0000FF"" >官方默认分类参考</font></a></td></tr></table><br>"
		  follows=0
		  else
		  do while not rs.EOF
		  i=i+1%>
        <FORM name="form1" method="post" action="store_Level1.asp?action=edit&id=<%=int(rs("id"))%>&idd=<%=rs("i")%>">
          <TR bgcolor="#FFFFFF" align="center">		  
		  <TD bgcolor="#F4F7F9"><%=rs("id")%></TD>
            <TD bgcolor="#F4F7F9"><INPUT name="dname" type="text" size="30" value="<%=rs("name")%>">
            &nbsp;[<A href="store_Level2.asp?id=<%=rs("id")%>">子目录</A>]</TD>
            <TD bgcolor="#F4F7F9"><INPUT name="indexid" type="text" size="3" value="<%=rs("indexid")%>"></TD>
...
<table width="98%" border="1" align="center" cellpadding="3" cellspacing="0" bordercolorlight="#335078" bordercolordark="#FFFFFF" bgcolor="#C5D5E4"  style="font-size:12px; letter-spacing:1px;">
  <tr> 
    <td height="20" bgcolor="#C5D5E4" align="center"><span style="color: #FF0000"><strong><font style="font-size:14px">添 加 一 级 分 类 栏 目</font></strong></span></td>
  </tr>
  <TR> 
    <TD bgcolor="#EAEEF4">
	<BR>
        ...
        <FORM name="form1" method="post" action="store_Level1.asp?action=add">
