...
rs.open "select * from DNJY_type where id="&id&" and twoid=0",conn,1,3
mname=rs("name")
Call CloseRs(rs)
...
set rs=server.CreateObject("adodb.recordset")
if dname<>"" then
rs.open "select * from DNJY_type where id="&id&" and twoid<>0 and threeid=0 and name='"&dname&"'",conn,1,3
if Not rs.eof Then
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from DNJY_type where id="&id&" order by twoid "&DN_OrderType&"",conn,1,3
two=rs("twoid")+1
rs.close
...
case "edit"
   set rs=server.CreateObject("adodb.recordset")
	rs.open "select name from DNJY_type where id="&id&" and name='"&dname&"' and i<>"&idd&"",conn,1,1
	if not rs.eof Then
	response.Write "<script language=javascript>alert('此分类名：["&dname&"]已经存在，请重新选择一个!');location.replace('?id="&id&"');</script>"	
	set rs=Nothing
    response.end	
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from DNJY_type where threeid=0 and twoid="&twoid&" and id="&id,conn,1,3
yname=rs("name")
rs("name")=dname
rs("indexid")=strint(request("indexid"))
rs("indexshow")=HtmlEncode(request("indexshow"))
...
case "del"
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_type where id="&id&" and threeid>0 and twoid ="&twoid
...
End If
set rs=server.CreateObject("adodb.recordset")
conn.execute ("delete from DNJY_type where id="&id&" and twoid="&twoid&"")
'删除关联分类的信息及相关文件开始
...
'删除关联分类的信息及相关文件END
response.Redirect "type_Level2.asp?id="&id
end select
%>
<HTML>
<HEAD>
...
<TABLE width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#799AE1">
  ...
  <TR> 
    <TD bgcolor="#FFFFFF"> &nbsp;[<A href="type_Level1.asp">首页</A>]-&gt;<%=mname%><BR>
	<TABLE width="98%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#D6DFF7">
        ...
        <%set rs=server.CreateObject("adodb.recordset")
		  rs.Open "select * from DNJY_type where id="&id&" and twoid<>0 and threeid=0 order by twoid",conn,1,1
		  dim follows
		  ...
		  i=i+1
		  %>
        <FORM name="form1" method="post" action="type_Level2.asp?action=edit&id=<%=int(rs("id"))%>&twoid=<%=int(rs("twoid"))%>&idd=<%=rs("i")%>">
          <TR bgcolor="#FFFFFF" align="center">
		  <TD><%=i%></TD>
            <TD><INPUT name="dname" type="text" size="30" value="<%=trim(rs("name"))%>">
            &nbsp;[<A href="type_Level3.asp?id=<%=rs("id")%>&twoid=<%=rs("twoid")%>">子目录</A>]</TD>
            <TD><INPUT name="indexid" type="text" size="3" value="<%=trim(rs("indexid"))%>"></TD>
...
<TABLE width="98%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#799AE1">
  ...
  <TR> 
	...
	<TABLE width="98%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#D6DFF7">
        ...
        <FORM name="form1" method="post" action="type_Level2.asp?action=add&id=<%=id%>">
