...
If conn.Execute("Select sms_del From DNJY_config")(0)>0 then'自动删除手机短信验证日志
Dim rsdq
set rsdq=server.createobject("adodb.recordset")
sql="delete from [DNJY_sms] where lock=0 and DateDiff("&DN_DatePart_D&",sms_addtime,"&DN_Now&")>"&conn.Execute("Select sms_del From DNJY_config")(0)&""'自动删除手机短信验证日志
...
sql="delete from [DNJY_sms] where lock=0 and id="&cstr(str2(i))
rs.open sql,conn,1,3
Next'循环结束
response.write "<script language=JavaScript>" & chr(13) & "alert('操作成功（若是锁定手机不能删除）。');" &"window.location='?isse=0'" & "</script>"
response.end
End If
...
sql="select * from DNJY_sms where id="&cstr(str2(i))
...
If request("lock")="shno"  Then Conn.Execute("Update DNJY_sms Set lock=0 where id="&cstr(Request("id")))
If request("lock")="sh"  Then Conn.Execute("Update DNJY_sms Set lock=1 where id="&cstr(Request("id")))

if isse=0 then
sql="select * from DNJY_sms order by id desc"
end if

if isse=1 Then
	if conditions=1 then		
	sql="select * from DNJY_sms where sms_mobile='"&searchkeys&"' order by id"
	end if
	if conditions=2 then		
	sql="select * from DNJY_sms where sms_addip='"&searchkeys&"' order by id"
	end If
	if conditions=3 then		
	sql="select * from DNJY_sms where lock=1 order by id"
	end if	
end if
%>
<html>
...
<body>
  ...
  <table width="90%" border="0" cellspacing="1" align="center" class="list">
    <tr>
      <td><table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td><form name="postart" method="post" action="sms_loglist.asp?isse=1" onSubmit="return checkform()">
              <br>
              查找信息：
  ...
  <table width="90%" border="0" cellspacing="1" align="center" class="list">
    <tr>
      <td><font color="#ff0000">注意：未锁定的手机验证日志将在<%If conn.Execute("Select sms_del From DNJY_config")(0)>0 then 
      response.write ""&conn.Execute("Select sms_del From DNJY_config")(0)&"天后"
	  Else
      response.write "设置时间内"
	  End if%>自动删除！</font></TD>
  </TR>
</table>
 <table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#ADAED6" width="100%">
  <form name="thisForm" action="?key=ok" method="POST">
    <TR> 
    <TD height="5" colspan="13" bgcolor="#799AE1" align="center"><FONT color="#FFFFFF" style="font-size:14px">手机验证日志管理</FONT></TD>
  </TR>
    ...
    <tr>
      ...
      <td width="50"><div align="center">验证码</div></td>
      <td width="100" ><div align="center">验证发起IP</div></td>
	  <td width="100" ><div align="center">验证时间</div></td>
      <td width="50" ><div align="center">是否锁定</div></td>
...
%>
    ...
    <tr>
      ...
      <td>
		...
		%></td>
      <td><a href="javascript:go('?key=ok&DelID=<%=rs("id")%>','您确定要删除？')">删除</a></td>
    </tr>
<%
rs.movenext
pp=pp+1
