...
%>
<form name="form1" method="post" action="admin_pinglundel.asp?action=delarticle&pl=yes">
<%
...
%>
            ...
            <tr>
              ...
              <td width="5%" align="left" bgcolor="#CCCCCC"><a href=admin_editComment.asp?Id=<%=Cstr(rs("pinglunid"))%> >编辑</a></td>
		      <td width="10%" align="left" bgcolor="#CCCCCC">审核: <%if rs("sh")=1 then%><a href=admin_pinglunsh.asp?action=shno&id=<%=rs("pinglunid")%>><font color=red>是</font></a><%else%><a href=admin_pinglunsh.asp?action=sh&id=<%=rs("pinglunid")%>>否</a><%end if%></td>
              <td width="18%" align="left" bgcolor="#CCCCCC"><%if session("aleave")="check" then%>删除<%else%><a href="javascript:DoEmpty('admin_pinglundel.asp?id=<%=rs("pinglunid")%>&url=2&pl=yes');">删除</a><%end if%></td>
            </tr>
            <tr>
