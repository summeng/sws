...
<form name="myform" method="POST" action="collect_Settings.asp">
        <TBODY>
        <TR>
...
SqlItem="select * from DNJY_xx_Filters order by FilterID "&DN_OrderType&""         
...
%>
        <TR class="tdbg">
          ...
          <TD class="tdbg" align="center">
            <%If Flag=True Then 
                Response.Write "<a href=collect_Settings.asp?Action=SetFlag&FlagName=a0&FilterID=" & FilterID & ">禁用</a>&nbsp;" & vbCrLf
            Else
                Response.Write "<a href=collect_Settings.asp?Action=SetFlag&FlagName=a1&FilterID=" & FilterID & ">启用</a>&nbsp;" & vbCrLf
            End If

            If PublicTf=True Then 
                Response.Write "&nbsp;&nbsp;<a href=collect_Settings.asp?Action=SetFlag&FlagName=b0&FilterID=" & FilterID & ">私有</a>&nbsp;" & vbCrLf
            Else
                Response.Write "&nbsp;&nbsp;<a href=collect_Settings.asp?Action=SetFlag&FlagName=b1&FilterID=" & FilterID & ">公有</a>&nbsp;" & vbCrLf
            End If			
			%>&nbsp;
             <a href="collect_modify.asp?FilterID=<%=RsItem("FilterID")%>">修改</a>
             &nbsp;<a href="collect_Settings.asp?Action=SetFlag&FlagName=Del&FilterID=<%=RsItem("FilterID")%>" onclick='return confirm("确定要删除此记录吗？");'>删除</a>
          </td>
          </TD>
        </TR>
<%    iItem=iItem+1
...
<%
Response.Write ShowPage("collect_Settings.asp",ItemNum,MaxPerPage,True,True," 个记录")
%>
...
Sub  SetFlag
   ...
   If FoundErr<>True Then
      ...
      Case "Del","删&nbsp;&nbsp;除"
         SqlItem="Delete from [DNJY_xx_Filters] Where FilterID In(" & FilterID & ")"
      Case "b0"
         SqlItem="Update [DNJY_xx_Filters] set PublicTf="&DN_False&" Where FilterID In(" & FilterID & ")"
      Case "b1"
         SqlItem="Update [DNJY_xx_Filters] set PublicTf="&DN_True&" Where FilterID In(" & FilterID & ")"
      Case "公&nbsp;&nbsp;有"
         SqlItem="Update [DNJY_xx_Filters] set PublicTf="&DN_True&" Where FilterID In(" & FilterID & ")"
      Case "私&nbsp;&nbsp;有"
         SqlItem="Update [DNJY_xx_Filters] set PublicTf="&DN_False&" Where FilterID In(" & FilterID & ")"
      Case "a0"
         SqlItem="Update [DNJY_xx_Filters] set Flag="&DN_False&" Where FilterID In(" & FilterID & ")"
      Case "a1"
         SqlItem="Update [DNJY_xx_Filters] set Flag="&DN_True&" Where FilterID In(" & FilterID & ")"
      Case "启&nbsp;&nbsp;用"
         SqlItem="Update [DNJY_xx_Filters] set Flag="&DN_True&" Where FilterID In(" & FilterID & ")"
      Case "禁&nbsp;&nbsp;用"
         SqlItem="Update [DNJY_xx_Filters] set Flag="&DN_False&" Where FilterID In(" & FilterID & ")"
