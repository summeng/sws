...
dim action,idd,dcity,dcityadmin,dcitypass,dbpath,ycity,fdel,iPage,adid,pic,sql1,sql2,sql3,API_QQCallBack
id=strint(request.QueryString("id"))
idd=strint(request.QueryString("idd"))
action=HtmlEncode(request("action"))
...
If dcity<>"" Then  
   set rs=server.CreateObject("adodb.recordset")
	rs.open "select city from DNJY_city where twoid=0 and threeid=0  and city='"&dcity&"' ",conn,1,1
	if not rs.eof Then
...
set rs=server.CreateObject("adodb.recordset")
		   rs.open "select * from DNJY_city where threeid=0 and twoid=0 order by id "&DN_OrderType&"",conn,1,1
...
sql="select username,password,useryz,zcdata,vipdq,city_oneid,city_one,city_twoid,city_threeid,cid from DNJY_user"
...
If dcity<>"" Then  
   set rs=server.CreateObject("adodb.recordset")
	rs.open "select city from DNJY_city where twoid=0 and threeid=0  and city='"&dcity&"' and i<>"&idd&"",conn,1,1
	if not rs.eof Then
	response.Write "<script language=javascript>alert('此地区名：["&dcity&"]已经存在，请重新选择一个!');location.replace('javascript:history.go(-1)');</script>"	
	set rs=Nothing
    response.end	
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from DNJY_city where twoid=0 and id="&id,conn,1,3
id=rs("id")
ycity=rs("city")
rs("city")=dcity
rs("dname")=dname
...
rs.Update
Call CloseRs(rs)
sql="update DNJY_info set city_one='"&dcity&"' where city_oneid="&id&" and city_twoid=0"
conn.execute sql

set rs = Server.CreateObject("ADODB.RecordSet")'如果前台没此会员同时注册前台会员
sql="select username,password,useryz,zcdata,vipdq,city_oneid,city_one,cid from DNJY_user where username='"&dcityadmin&"'"
rs.open sql,conn,1,3
...
case "del"
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_city where twoid>0 and id ="&id
rs.open sql,conn,3,2
if not rs.eof Then
response.Write "<script language=javascript>alert('此地区下级分类非空，请先删除下级分类才能删除本分类!');location.replace('?');</script>"
set rs=Nothing
response.End
End If

conn.execute ("delete from DNJY_city where id="&id)

'删除关联地区的信息及相关文件开始
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_info where city_oneid ="&id
rs.open sql,conn,3,2
...
'删除关联地区的新闻公告及相关文件开始
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_news where city_oneid ="&id
rs.open sql,conn,3,2
if not rs.eof then
 For iPage = 1 To rs.recordcount
 adid=rs("id")
...
'删除关联地区的都市114开始
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_114 where city_oneid ="&id
rs.open sql,conn,3,2
if not rs.eof then
For iPage = 1 To rs.recordcount
rs.delete
...
'删除关联地区的留言开始
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_gbook where city_oneid ="&id
rs.open sql,conn,3,2
if not rs.eof then
For iPage = 1 To rs.recordcount
rs.delete
...
'删除关联地区的友情链接开始
set rs=server.CreateObject("adodb.recordset")
sql="select * from DNJY_link where city_oneid ="&id
rs.open sql,conn,3,2
if not rs.eof then
For iPage = 1 To rs.recordcount
rs.delete
...
set fdel=nothing
          Call CloseRs(rs)
		  Call CloseDB(conn) 
response.Redirect "city_Level1.asp" 
end select
%>
<HTML>
  ...
  <TR> 
    <TD bgcolor="#FFFFFF"> &nbsp;&nbsp;首页<BR>
	  <TABLE width="98%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#D6DFF7">
        ...
        <%set rs=server.CreateObject("adodb.recordset")
		  rs.Open "select * from DNJY_city where id>0 and twoid=0 order by id",conn,1,1
		  dim follows
		  if rs.EOF and rs.BOF then
          response.write"<tr bgcolor=#FFFFFF><td colspan='9'><p align='center'><font color='red'>暂无地区分类！请先添加分类。如果觉得官方默认的<a href=""http://test.ip126.com/city_list.asp"" target=_blank><FONT color=""#0000FF"" >“省、市、县”</font></a>三级分类合用，可</font><a href=""Classify_in_out.asp?Action=cityfl"" TARGET=right><FONT color=""#0000FF"" >导入官方默认地区分类。</font></a>如果不合用请勿导入，否则一个个删除非常麻烦！<a href=""http://test.ip126.com/city_list.asp"" target=_blank><FONT color=""#0000FF"" >官方默认分类样式参考</font></a></td></tr></table><br>"
		  follows=0
		  Else		  
		  do while not rs.EOF
				WebSetting = Empty
		  ...
		  i=i+1
		  %>
        <FORM name="form1" method="post" action="?action=edit&id=<%=int(rs("id"))%>&idd=<%=rs("i")%>">
          <TR bgcolor="#FFFFFF" align="center"> 
            <TD><%=i%></TD>
			<TD><%=trim(rs("id"))%></TD>
            <TD>
              <INPUT name="city" type="text" id="city" value="<%=trim(rs("city"))%>" size="30">
              [<A href="city_Level2.asp?id=<%=rs("id")%>">子目录</A>]</TD>
            <TD>
			...
			<INPUT type="submit" name="Submit" value="修 改">			 
              &nbsp; 
              <INPUT type="button" name="DEL" onClick="{if(confirm('确定要删除这个地区分类吗？\n将删除此分类下所有信息\n此操作不可以恢复！')){location.href='?city=<%=rs("city")%>&action=del&id=<%=rs("id")%>';}return false;}" value="删除" >
			    &nbsp;&nbsp;<input type="button" value="属 性" onClick="show('s<%=Rs("id")%>');">
            </TD>
          </TR>
          <TR bgcolor="#ffffff" align="center" id="s<%=Rs("id")%>" style="display:none">
           <TD colspan="9">
					 <table width="700" border="0" cellpadding="0" cellspacing="0" align="center">
							...
							<TR bgcolor="#FFFFFF" align="center">
							 ...
							 <TD align="left" bgcolor="#C2D3FC"><input type="text" name="w12" type="text" id="w0" value="<%=WebSetting(12)%>"></TD>
						  </TR>
							<TR bgcolor="#FFFFFF" align="center">
