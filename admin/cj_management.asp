...
Dim ItemID,ItemName,city_oneid,city_twoid,city_threeid,city_one,city_two,city_three,c_oneid,c_twoid,c_threeid,c_one,c_two,c_three,ListStr,ListPaingType,ListPaingStr2,ListPaingID1,ListPaingID2,ListPaingStr3,Flag
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
  <tr class="tdbg"> 
    ...
    <td height="30"><a href=cj_management.asp>管理首页</a> | <a href="cj_add_a.asp">添加新项目</a></td>
  </tr>
</table>
<br>
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border" >
...
<table class="border" border="0" cellspacing="1" width="100%" cellpadding="0">
  <form name="myform" method="POST" action="cj_management.asp">
...
SqlItem="select * from DNJY_Article_Item order by ItemID "&DN_OrderType&""
...
%>
    <tr class="tdbg" onMouseOut="this.style.backgroundColor=''" onMouseOver="this.style.backgroundColor='#BFDFFF'" style="padding: 0px 2px;" align="center" height="22">
      ...
      <td width="120"><%=c_one%><%If c_two<>"" Then Response.Write " / "%><%=c_two%><%If c_three<>"" Then Response.Write " / "%><%=c_three%></td>
      ...
      <td width="157">
       ...
       End If
       %>      </td>
      <td width="148"><a href="cj_editor_a.asp?ItemID=<%=ItemID%>">编辑</a> <a href="cj_attribute.asp?ItemID=<%=ItemID%>">属性</a> <a href="cj_editor_e.asp?ItemID=<%=ItemID%>">测试</a> <a href="cj_copy.asp?Action=Copy&ItemID=<%=ItemID%>">复制</a>  <a href="cj_management.asp?Action=Del&ItemID=<%=ItemID%>" onClick='return confirm("确定要删除此项目吗？请您慎重选择！这将删除该项目的项目信息，历史记录及过滤信息 3 个项目类型数据。");'>删除</a></td>
    </tr>
    <%iItem=iItem+1
...
<%
Response.Write ShowPage("cj_management.asp",ItemNum,MaxPerPage,True,True," 个项目")
%>
...
If Request("Del")="清空所有记录" Then
   conn.Execute("Delete From DNJY_Article_Item")
   conn.Execute("Delete From DNJY_Article_Filters")
   conn.Execute("Delete From DNJY_Article_Histroly")
Else
   If ItemID="" Then
      FoundErr=True
   ...
   Else
      ...
      conn.Execute("Delete From [DNJY_Article_Item] Where ItemID In(" & ItemID & ")")
      conn.Execute("Delete From [DNJY_Article_Filters] Where ItemID In(" & ItemID & ")")
      conn.Execute("Delete From [DNJY_Article_Histroly] Where ItemID In(" & ItemID & ")")
   End If
End  If
End Sub
%>
