...
Title=rs("title")
Content=rs("Content")
If IsNull(rs("Content")) Then Content="暂无内容..."
addtime=rs("addtime")
author=rs("author")
origin=rs("origin")
...
<form method="POST" action="UserNews_ModifySave.asp?isse=<%=isse%>&id=<%=id%>&page=<%=page%>&searchkeys=<%=searchkeys%>&selecttype=<%=selecttype%>&se_ClassName=<%=se_ClassName%>" name="postart" onSubmit="return checkForm()">
  ...
  <!---上部通栏广告结束----> 
	...
	<TR> 
    <TD height="5" colspan="8" bgcolor="#799AE1" align="center"><FONT color="#FFFFFF" style="font-size:14px">修改信息--该信息ID为：<b><%=ID%></FONT></TD>
    </TR>
     <tr> 
...
function changelocation(locationid)
	...
	document.addNEWS.city_twoid.options[0] = new Option('选择城市','');
	document.addNEWS.city_threeid.length = 0; 
	document.addNEWS.city_threeid.options[0] = new Option('选择城市','');
...
%>
                ...
                <tr> 
                  <td width="10%" align="right" valign="middle"><div align="center">信息类别：</div></td>
...
%>
      ...
      color=#993300>互联网</FONT></FONT>】</td>
                ...
                不共享<%end if%>  (设置共享后可在总站相关栏目显示)
                </td>
                </tr>                 
                
                <tr> 
                  <td height="20" align=center ><div align="center">文章内容：</div></td>
				  <td height="20" colspan="2"  ><textarea id="editor" name="d_content" style="width:670px;height:400px;"><%=Content%></textarea><input type="checkbox" name="T_SaveImg" value="1" /> 保存内容中远程图片到本地&nbsp;&nbsp;<input type="checkbox" name="AspJpeg" value="1" checked/>给上传图片加水印<br></td>
                </tr>
                <tr>
